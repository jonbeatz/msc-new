...
| **RP-2026-04-12-branch-tip-ready-tomorrow** | 2026-04-12 | **Current known-good tip (use this to resume):** **`mscNowLive-v4-RestorePoint`** @ **`929cf04`**. **Code line:** **`38dd8f8`** — homepage hero from **`globals/Homepage`** + **Media** ( **`collections/HeroSlides.ts` removed** ); **`lib/email-brand.ts`**, **`lib/email-templates.ts`**; **`payload-types.ts`** + section/CMS updates. **Ops/docs:** **`fbb6afd`** ReCall closeout; **`b92d1be`** **`parity-ftp-report.md`** gitignored; **`929cf04`** this doc sync (**Restore-Points**, **ReCall** Current focus, **Ready to begin** notes in **START-HERE** / **§0** / **Jedi-List**). **Live:** Tier 2 **`npm run pushit:live`** completed same session (admin-ui + `.next` + **`payload.sqlite`** + **`public/media`**). **Restore:** `git fetch origin && git checkout mscNowLive-v4-RestorePoint && git pull && git reset --hard 929cf041c1268559a2dabc6727ef3eebe1d5ad02`. **Tomorrow:** paste **`Ready to begin`** from **Agent-Runbook §0** (or **Custom-Prompts** item **0**); then **`npm run dev:fresh`** if **`/`** or **`/admin`** are not healthy. |
